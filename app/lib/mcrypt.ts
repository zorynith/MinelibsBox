...
function md5Core(bytes: Uint8Array): Uint8Array {
...
/** 计算字符串的 MD5 (与 PHP md5() 一致, UTF-8 编码)。 */
...
function md5Hex(bytes: Uint8Array): string {
  let hex = "";
  for (const b of md5Core(bytes)) hex += b.toString(16).padStart(2, "0");
  return hex;
}

/** HMAC-MD5 (与 PHP hash_hmac('md5', data, key) 一致)。 */
export function hmacMd5(key: string, msg: string): string {
  const enc = new TextEncoder();
  let block = new Uint8Array(64);
  const keyBytes = enc.encode(key);
  block.set(keyBytes.length > 64 ? md5Core(keyBytes) : keyBytes);

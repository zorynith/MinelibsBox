...
/** 规范化分享源路径（相对路径，去掉首尾多余斜杠，目录保留尾斜杠）。{虚拟前缀} 路径不加前导斜杠。 */
...
export async function resolveShareSource(
  ...
  const pub = parsePublishPath(path);
  if (pub) {
    if (isFolder) {
...
export function parsePublishPath(path: string): { userID: number; name: string; key: string } | null {
  const m = (path || "").match(/^\{publish:(\d+):([^}]+)\}/);
  if (!m) return null;
  return { userID: parseInt(m[1], 10), name: m[2], key: `__publish__/${m[1]}/${m[2]}` };
}

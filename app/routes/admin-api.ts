...
import { ioClientFromConfig } from "../lib/io";
...
// 对象存储驱动 (001 ioGroupType 'obj'), R2 对照 minio
const OBJ_DRIVERS = ["s3", "oss", "cos", "qiniu", "minio", "oos", "uss", "eos", "eds", "obs", "jos", "moss", "nos"];
const NET_DRIVERS = ["ftp", "webdav"];

/** 001 ioGroupType: 存储类别 (loc/obj/net/oth) */
function ioGroupType(driver: string): string {
  const d = String(driver || "").toLowerCase();
  if (d === "local") return "loc";
  if (OBJ_DRIVERS.includes(d)) return "obj";
  if (NET_DRIVERS.includes(d)) return "net";
  return "oth";
}

/** 前端默认值判断: "1"/1/true 视为真 ("0"/""/undefined 为假) */
function isTrue(v: unknown): number {
  return v === 1 || v === true || String(v) === "1" ? 1 : 0;
}

// 默认存储(R2)已用空间统计缓存 (后台低频, 60s 有效)
const storageUseCache = new Map<string, { size: number; fileNum: number; ts: number }>();
async function r2TotalUsed(bucket: R2Bucket): Promise<{ size: number; fileNum: number }> {
  const hit = storageUseCache.get("ALL");
  if (hit && Date.now() - hit.ts < 60_000) return hit;
  let size = 0, fileNum = 0, cursor: string | undefined;
  try {
    do {
  ...
  storageUseCache.set("ALL", { size, fileNum, ts: Date.now() });
  return { size, fileNum };
}
...
adminApi.all("/storage/get", async (c) => {
  ...
  for (const item of list) {
    let sizeUse = 0, fileNum = 0;
    const isDefault = parseInt(String(item.is_default ?? "0"), 10) === 1;
    const isSystem = parseInt(String(item.system ?? "0"), 10) === 1;
    if (isDefault || isSystem) {
      // R2 本地存储: 全 bucket 扫描统计 (排除隐藏/临时对象)
      const used = await r2TotalUsed(c.env.FILES);
      sizeUse = used.size;
      fileNum = used.fileNum;
    } else if (String(item.driver || "").toLowerCase() !== "minio") {
      // S3 外链存储: 通过 S3 list-all 统计 (带缓存)
      const cacheKey = `storage:${item.id}`;
      const hit = storageUseCache.get(cacheKey);
      if (hit && Date.now() - hit.ts < 60_000) {
        sizeUse = hit.size;
        fileNum = hit.fileNum;
      } else {
        let config: Record<string, unknown> = {};
        try { config = JSON.parse(String(item.config || "{}")); } catch { config = {}; }
        const basePath = String((config as any).basePath || "").replace(/^\/+|\/+$/g, "");
        try {
          const io = ioClientFromConfig(String(item.driver || ""), config);
          if (!io) throw new Error("incomplete io config");
          const all = await io.listAll(basePath ? basePath + "/" : "");
          for (const o of all) { sizeUse += o.size; fileNum++; }
          storageUseCache.set(cacheKey, { size: sizeUse, fileNum, ts: Date.now() });
        } catch { /* 外部存储不可达时按 0 */ }
      }
    }
    out.push({
      ...
      name: item.name,
      driver: item.driver,
      // 001 原版 GB 语义: DB 以 GB 数值存储, 前端 userSpaceHtml/sizeDisplay 按 ×1024³ 还原字节显示
      ...
      groupType: ioGroupType(String(item.driver || "")),
    });
  }
  return c.json(ok(out));
});

/** secret 等敏感配置打码 (001 storage getConfig) */
function maskSecret(config: Record<string, any>): Record<string, any> {
  const out: Record<string, any> = { ...config };
  for (const key of ["secret", "userpass", "password"]) {
    if (typeof out[key] === "string" && out[key]) {
      out[key] = "*".repeat(Math.min(out[key].length, 20));
    }
  }
  return out;
}
...
/**
 * 存储配置 - admin/storage/getConfig
 * 参数: id; 返回 config 对象 (secret 打码)
 */
adminApi.all("/storage/getConfig", async (c) => {
  const q = await allParams(c);
  const id = parseInt(String(q.id || "0"), 10);
  const item = await getIoSourceById(c.env.DB, id);
  if (!item) return c.json(fail("explorer.error"));
  let config: Record<string, any> = {};
  try { config = JSON.parse(String(item.config || "{}")); } catch { /* ignore */ }
  return c.json(ok(maskSecret(config)));
});

/**
 * 添加存储 - admin/storage/add
 * 参数: name/sizeMax(GB)/driver/default/system/config(JSON 串)
 * sizeMax 前端表单为 GB, 与 001 原版一致: DB 直接以 GB 数值存储 (groups/users/io 统一 GB 语义)
 */
adminApi.all("/storage/add", async (c) => {
  const q = await allParams(c);
  const name = String(q.name || "").trim();
  const driver = String(q.driver || "").trim();
  if (!name || !driver) return c.json(fail("explorer.error"));
  const sizeMax = parseFloat(String(q.sizeMax || "0")) || 0;
  let config: Record<string, any> = {};
  try { config = JSON.parse(String(q.config || "{}")); } catch { /* ignore */ }
  const now = Math.floor(Date.now() / 1000);
  const res = await c.env.DB.prepare(
    `INSERT INTO io_source (name, driver, size_max, is_default, system, config, status, add_time, edit_time)
     VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?)`
  ).bind(name, driver, sizeMax, isTrue(q.default), isTrue(q.system), JSON.stringify(config), now, now).run();
  return c.json(ok("explorer.success", res.meta.last_row_id ?? null));
});

/**
 * 编辑存储 - admin/storage/edit
 * 参数: id + 可选字段; secret 打码值(全*)提交时保留原值
 */
adminApi.all("/storage/edit", async (c) => {
  const q = await allParams(c);
  const id = parseInt(String(q.id || "0"), 10);
  ...
  if (q.name !== undefined) { fields.push("name = ?"); values.push(String(q.name)); }
  if (q.driver !== undefined) { fields.push("driver = ?"); values.push(String(q.driver)); }
  if (q.sizeMax !== undefined) {
    // 001 原版 GB 语义: 直接以 GB 数值存储
    fields.push("size_max = ?");
    values.push(parseFloat(String(q.sizeMax)) || 0);
  }
  if (q.default !== undefined) { fields.push("is_default = ?"); values.push(isTrue(q.default)); }
  if (q.config !== undefined) {
    let config: Record<string, any> = {};
    try { config = JSON.parse(String(q.config)); } catch { /* ignore */ }
    // secret 打码值不覆盖原配置
    for (const key of ["secret", "userpass", "password"]) {
      if (typeof config[key] === "string" && config[key].length > 0 && config[key].split("").every((ch) => ch === "*")) {
        delete config[key];
      }
    }
    let oldConfig: Record<string, any> = {};
    try { oldConfig = JSON.parse(String(item.config || "{}")); } catch { /* ignore */ }
    fields.push("config = ?");
    values.push(JSON.stringify({ ...oldConfig, ...config }));
  }
  if (fields.length > 0) {
    fields.push("edit_time = ?");
    values.push(Math.floor(Date.now() / 1000));
    values.push(id);
    await c.env.DB.prepare(`UPDATE io_source SET ${fields.join(", ")} WHERE id = ?`).bind(...values).run();
  }
  storageUseCache.delete("ALL");
  return c.json(ok("explorer.success"));
});

/**
 * 删除存储 - admin/storage/remove
 * 简化: 系统默认存储(id=1)禁止删除; 其余直接删记录 (无 File 表迁移)
 */
adminApi.all("/storage/remove", async (c) => {
  const q = await allParams(c);
  const id = parseInt(String(q.id || "0"), 10);
  if (!id) return c.json(fail("explorer.error"));
  const item = await getIoSourceById(c.env.DB, id);
  if (!item) return c.json(fail("explorer.error"));
  if (parseInt(String(item.system ?? "0"), 10) === 1 || parseInt(String(item.is_default ?? "0"), 10) === 1) {
    return c.json(fail("系统默认存储不可删除"));
  }
  await c.env.DB.prepare("DELETE FROM io_source WHERE id = ?").bind(id).run();
  return c.json(ok("explorer.success"));
});
...
adminApi.all("/member/remove", async (c) => {
  ...
  // 清理 R2 存储: 删除用户空间全部对象 (防止删除账号后 R2 残留用户文件夹)
  const users = await c.env.DB.prepare(`SELECT id, username FROM users WHERE id IN (${placeholders})`).bind(...ids).all<{ id: number; username: string }>();
  for (const u of users.results) {
    await deleteDirectory(c.env.FILES, `${u.username}/`).catch(() => {});
  }

  await c.env.DB.prepare(`DELETE FROM shares WHERE user_id IN (${placeholders})`).bind(...ids).run();
  await c.env.DB.prepare(`DELETE FROM share WHERE userID IN (${placeholders})`).bind(...ids).run();
...
// ============ admin/plugin (插件管理) ============
// Mirrors 001 admin/plugin controller: appList / changeStatus / getConfig / setConfig / install / unInstall.
// 无服务器环境无在线商店与物理目录, install 返回内置提示; unInstall 降级为禁用(可逆)。

/** 插件启用时检查必填配置缺失 (mirrors 001 changeStatus needConfig 检测). */
function pluginNeedConfig(pkg: any, config: Record<string, any>): boolean {
  const items = pkg.configItem || {};
  for (const [key, item] of Object.entries<any>(items)) {
    if (!item || typeof item !== "object") continue;
    if (!item.require) continue;
    const hasDefault = item.value !== undefined && item.value !== "" && item.value !== null;
    const hasConfig = config[key] !== undefined && config[key] !== "" && config[key] !== null;
    if (!hasDefault && !hasConfig) return true;
  }
  return false;
}
...
adminApi.all("/plugin/appList", async (c) => {
  ...
  const data = await buildPluginAppList(c.env.ASSETS, c.env.DB, lang, getStaticHost(c));
  return c.json({ code: true, data });
});

adminApi.all("/plugin/changeStatus", async (c) => {
  const body = await allParams(c);
  const app = body.app;
  const status = Number(body.status) ? 1 : 0;
  if (!app) return c.json(fail("explorer.dataNotFull"));
  const pkg = await resolvePluginPackage(c.env.ASSETS, app, detectLang(c));
  if (!pkg) return c.json(fail("admin.plugin.disabled"));
  if (status === 1) {
    const meta = await getPluginMeta(c.env.DB, app);
    if (pluginNeedConfig(pkg, meta.config)) {
      return c.json({ code: false, data: "needConfig" });
    }
  }
  await setPluginStatus(c.env.DB, app, status);
  const data = await buildPluginAppList(c.env.ASSETS, c.env.DB, detectLang(c), getStaticHost(c));
  return c.json({ code: true, data });
});

adminApi.all("/plugin/getConfig", async (c) => {
  const body = await allParams(c);
  const app = body.app;
  if (!app) return c.json(fail("explorer.dataNotFull"));
  const pkg = await resolvePluginPackage(c.env.ASSETS, app, detectLang(c));
  ...
  const userSelectInfo = { type: "mutil", user: "mutil", group: "mutil", role: "mutil" };
  // 配置项 display/desc 等文案多为系统 admin 语言 key(如 admin.plugin.auth),
  // 插件语言包不含这些, 需用系统语言包补充解析残留的 {{LNG['...']}} 占位符
  const sysLang = (await loadLangPack(c.env.ASSETS, detectLang(c))) || {};
  // 通用配置项默认文案(001 生态插件系统内置, 不随插件 package.json 分发):
  // 插件复刻时缺省 display/desc/info 会导致配置页出现"无标题选项", 按 key 补齐
  const commonConfigDefault: Record<string, { display?: string; desc?: string; info?: Record<string, string> }> = {
    pluginAuth: { display: "{{LNG['admin.plugin.auth']}}", desc: "{{LNG['admin.plugin.authDesc']}}" },
    openWith: {
      display: "{{LNG['admin.plugin.openWith']}}",
      info: { dialog: "{{LNG['admin.plugin.openWithDilog']}}", window: "{{LNG['admin.plugin.openWithWindow']}}" },
    },
    fileExt: { display: "{{LNG['admin.plugin.fileExt']}}", desc: "{{LNG['admin.plugin.fileExtDesc']}}" },
  ...
  for (const [key, item] of Object.entries<any>(pkg.configItem || {})) {
    ...
    if (def) {
      if (!merged.display && def.display) merged.display = def.display;
      if (!merged.desc && def.desc) merged.desc = def.desc;
      if (def.info) merged.info = { ...def.info, ...(merged.info || {}) };
    }
    const copy = resolveLngRaw(merged, sysLang);
    if (!["html", "button"].includes(copy.type) && meta.config[key] !== undefined) {
      // 规范化文件扩展名值(去点/小写/去重), 避免 tags 控件回写 ".pdf,.PDF" 脏值
      copy.value = normalizePluginConfig({ [key]: meta.config[key] })[key];
    }
    if (copy.type === "userSelect" && !copy.info) {
      copy.info = userSelectInfo;
    }
    formData[key] = copy;
  }
  return c.json({ code: true, data: formData });
});

adminApi.all("/plugin/setConfig", async (c) => {
  const body = await allParams(c);
  const app = body.app;
  const raw = body.value;
  if (!app || !raw) return c.json(fail("explorer.dataNotFull"));
  const pkg = await resolvePluginPackage(c.env.ASSETS, app, detectLang(c));
  if (!pkg) return c.json(fail("admin.plugin.disabled"));
  let config: Record<string, any>;
  if (raw === "reset") {
    config = defaultPluginConfig(pkg);
  } else {
    try {
      config = JSON.parse(raw);
    } catch {
      return c.json({ code: false, data: raw });
    }
...
adminApi.all("/plugin/install", async (c) => {
  ...
  if (!app || !step) return c.json(fail("explorer.dataNotFull"));
  return c.json({ code: false, data: "内置插件已安装, 在线安装/更新商店功能暂不支持" });
});

adminApi.all("/plugin/unInstall", async (c) => {
  const body = await allParams(c);
  const app = body.app;
  if (!app) return c.json(fail("explorer.dataNotFull"));
  await setPluginStatus(c.env.DB, app, 0);
  const data = await buildPluginAppList(c.env.ASSETS, c.env.DB, detectLang(c), getStaticHost(c));
  return c.json({ code: true, data });
});

// ============ admin/notice 公告管理 ============
// 复刻 001 adminNotice 控制器; 用户端展示见 user/setting/notice。

function noticeAuthCheck(user: import("../lib/auth").AuthUser, auth: string): boolean {
...
// 公告列表 (管理员管理页)
adminApi.all("/notice/get", async (c) => {
  const user = c.get("currentUser");
  if (!isAdmin(user)) return c.json(fail("explorer.noPermissionAction"));
  const rows = await c.env.DB.prepare("SELECT * FROM notice ORDER BY sort ASC, id ASC").all<Record<string, unknown>>().catch(() => ({ results: [] }));
  return c.json(ok((rows.results || []).map(noticeRow)));
});

// 添加公告
adminApi.all("/notice/add", async (c) => {
  const user = c.get("currentUser");
  if (!isAdmin(user)) return c.json(fail("explorer.noPermissionAction"));
  const p = await allParams(c);
  if (!p.name || !p.content) return c.json(fail("explorer.dataNotFull"));
  const ts = Math.floor(Date.now() / 1000);
  const res: any = await c.env.DB.prepare(
    `INSERT INTO notice (name, content, auth, mode, time, type, level, enable, sort, createTime, modifyTime)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?)`
  )
    .bind(
      p.name,
      ...
      p.auth || "",
      p.mode || "0",
      parseInt(p.time, 10) || ts,
      parseInt(p.type, 10) || 1,
      parseInt(p.level, 10) || 0,
      p.enable === "1" || p.enable === "true" ? 1 : 0,
      ts,
      ts
    )
    .run();
  const id = Number(res.meta?.last_row_id ?? 0);
  const row: any = await c.env.DB.prepare("SELECT * FROM notice WHERE id = ?").bind(id).first().catch(() => null);
  return c.json(ok("explorer.success", row ? noticeRow(row) : { id }));
});

// 编辑公告
adminApi.all("/notice/edit", async (c) => {
  const user = c.get("currentUser");
  if (!isAdmin(user)) return c.json(fail("explorer.noPermissionAction"));
  const p = await allParams(c);
  const id = parseInt(p.id, 10) || 0;
  if (!id) return c.json(fail("explorer.dataNotFull"));
  if (!p.name || !p.content) return c.json(fail("explorer.dataNotFull"));
  await c.env.DB.prepare(
    `UPDATE notice SET name = ?, content = ?, auth = ?, mode = ?, time = ?, type = ?, level = ?, enable = ?, modifyTime = ? WHERE id = ?`
  )
    .bind(
      p.name,
      p.content,
...
// 删除公告
adminApi.all("/notice/remove", async (c) => {
  const user = c.get("currentUser");
  if (!isAdmin(user)) return c.json(fail("explorer.noPermissionAction"));
  const p = await allParams(c);
  const id = parseInt(p.id, 10) || 0;
...
// 排序
adminApi.all("/notice/sort", async (c) => {
  const user = c.get("currentUser");
  if (!isAdmin(user)) return c.json(fail("explorer.noPermissionAction"));
  const p = await allParams(c);
  const ids = (p.ids || "").split(",").map(Number).filter((n) => Number.isFinite(n) && n > 0);
  for (let i = 0; i < ids.length; i++) {
    await c.env.DB.prepare("UPDATE notice SET sort = ? WHERE id = ?").bind(i, ids[i]).run();
  }
  return c.json(ok("explorer.success"));
});

// 启/禁用公告
adminApi.all("/notice/enable", async (c) => {
  const user = c.get("currentUser");
  if (!isAdmin(user)) return c.json(fail("explorer.noPermissionAction"));
  const p = await allParams(c);
  const id = parseInt(p.id, 10) || 0;

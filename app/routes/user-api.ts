...
userApi.post("/index/loginSubmit", async (c) => {
  ...
  // 登录时若显式设置了 kodUserLanguage cookie, 持久化到用户配置(跨会话/设备兜底)。
  const cookieHeader = c.req.header("cookie") || "";
  const cookieLangRaw = (cookieHeader.match(/(?:^|;\s*)kodUserLanguage=([^;]+)/) || [])[1] || "";
  const cookieLang = cookieLangRaw ? normalizeLang(cookieLangRaw) : "";
  if (cookieLang) {
    try {
      const cfg = JSON.parse(String(user.config_json || "{}"));
      if (typeof cfg === "object" && cfg !== null && cfg.language !== cookieLang) {
        cfg.language = cookieLang;
        await c.env.DB.prepare("UPDATE users SET config_json = ? WHERE id = ?")
          .bind(JSON.stringify(cfg), user.id)
          .run()
          .catch(() => {});
      }
    } catch {
...
const userSpaceCache = new Map<string, { sizeUse: number; ts: number }>();
const USER_SPACE_TTL = 10_000;

async function userSpaceUsed(env: Env, username: string): Promise<number> {
  const hit = userSpaceCache.get(username);
  if (hit && Date.now() - hit.ts < USER_SPACE_TTL) return hit.sizeUse;
  let size = 0;
  try {
    ...
    do {
      ...
      for (const o of listed.objects) size += o.size;
      cursor = listed.truncated ? listed.cursor : undefined;
    } while (cursor);
  } catch {
    // R2 不可用时不阻塞登录
  }
  userSpaceCache.set(username, { sizeUse: size, ts: Date.now() });
  return size;
}
...
userApi.get("/view/options", async (c) => {
  ...
  // Check session
  const defaultIo = await getDefaultIoSource(c.env.DB);
  const defaultSizeMax = defaultIo ? parseInt(String((defaultIo as any).size_max ?? "0"), 10) : 0;
  const defaultTargetSpace = { sizeMax: defaultSizeMax * 1024 * 1024 * 1024, sizeUse: 0 };
  ...
  let userObj: any = {
    ...
    isRootAllowIO: 0,
    isRootAllowAll: 1,
    targetSpace: defaultTargetSpace,
  ...
  if (sessionId) {
    ...
    if (session) {
      ...
      const userSpaceUsedBytes = await userSpaceUsed(c.env, sessionUsername);
      ...
      userObj = {
        ...
        isOpenSafeSpace: (await getUserOption(c.env.DB, session.user_id as number, "safe_open", "safe")) === "1",
      };
    }
  }
...
userApi.get("/view/lang", async (c) => {
  ...
  // 仅当用户显式设置了 kodUserLanguage cookie 时才持久化语言到 DB,
  // 避免把 Accept-Language 推断的语言固化, 覆盖用户真实设置。
  const cookieHeader = c.req.header("cookie") || "";
  const cookieLangRaw = (cookieHeader.match(/(?:^|;\s*)kodUserLanguage=([^;]+)/) || [])[1] || "";
  const cookieLang = cookieLangRaw ? normalizeLang(cookieLangRaw) : "";
  if (cookieLang) {
    ...
    if (sessionId) {
      ...
      if (session && session.user_id) {
        const row = await getUserById(c.env.DB, session.user_id as number);
        if (row) {
          try {
            ...
            if (typeof cfg === "object" && cfg !== null && cfg.language !== cookieLang) {
              cfg.language = cookieLang;
              await c.env.DB.prepare("UPDATE users SET config_json = ? WHERE id = ?")
                .bind(JSON.stringify(cfg), session.user_id)
                .run()
                .catch(() => {});
            }
          } catch {
...
// ============ taskAction (user/setting + user/view) ============
// 复刻 001 user/setting/taskAction -> admin.task.taskActionRun 语义:
//  1) 任务已完成的结果缓存 (result_<longTaskID>, 由 unzipList 等写入) -> task_finished
//  2) task 表任务记录 (数字 id)
//  3) 无任务 -> taskEmpty (前端 notifyView 据此重轮询, 等待结果缓存写入)
async function taskActionHandler(c: any): Promise<Response> {
  const params: Record<string, string> = {};
  const body = await c.req.parseBody().catch(() => ({}));
  for (const [k, v] of Object.entries(body)) params[k] = typeof v === "string" ? v : "";
  for (const [k, v] of Object.entries(c.req.query())) params[k] = typeof v === "string" ? v : "";

  const action = params.action || "";
  const id = params.id || "";
  if (!["get", "kill", "stop", "restart"].includes(action)) return c.json({ code: false, data: "common.notExists" });

  if (action === "get") {
    const cached = await taskResultGet(c.env.DB, id);
    if (cached !== undefined) {
      let data: any = cached;
      try {
      ...
      return c.json({ code: true, data, info: "task_finished" });
    }
  }

  let row: any = null;
  if (/^\d+$/.test(id)) {
    row = await c.env.DB.prepare("SELECT * FROM task WHERE id = ?").bind(parseInt(id, 10)).first().catch(() => null);
  }
  if (!row) return c.json({ code: false, data: "common.notExists", info: "taskEmpty" });

  const now = Math.floor(Date.now() / 1000);
  let result: any = row;
  if (action !== "get") {
    const status = action === "stop" ? "stop" : action === "restart" ? "waiting" : "kill";
    await c.env.DB.prepare("UPDATE task SET status = ?, timeUpdate = ? WHERE id = ?").bind(status, now, row.id).run().catch(() => null);
    result = true;
  }
  return c.json({ code: true, data: result, info: row.status === "finished" ? "task_finished" : undefined });
}

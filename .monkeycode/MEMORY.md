...
- Instructions:
  ...
  - 仅当用户明确要求创建分支时才创建；不得自行判断或套用外部分支规范。
   ...
   - admin 密码/设置"变回默认"排查：后端 initDatabase/seed/migrations 全幂等（INSERT OR IGNORE / CREATE TABLE IF NOT EXISTS），无任何自动重置逻辑；唯一能把 admin 重置为 admin123 的路径是 admin 用户被删除后 seed 重建。`admin/member/remove` 曾只排除 id=1，admin 若 id≠1 可被误删（重建后密码变回 admin123、user_option 因新 id 失效）——已加固为禁止删除 role in ('admin','root') 的用户。线上改密/重置记录在 `audit_logs`（action=user.setUserInfo / user.findPassword / admin edit user），可据此核对时间线。线上核对铁证：seed 首用户 admin 应为 userID=1，若线上 admin userID 远大于 1（实测 1418）即证明被单行 DELETE 后重建过（AUTOINCREMENT 不回退）；用 admin123 登录后可调 `GET /api/admin/member/get?groupID=1` 看全量用户 id。另发现回收站开关读写 type 不一致 bug：`user/setting/setConfig` 与 `settingDefault` 写 user_option 时 type 均为空串，explorer 删除接口却用 `getUserOption(...,"recycleOpen","config")` 读取 → 永远读不到、回收站恒为开启（表现为"关不掉/设置变回"）；读取处已改 type 空串并兼容历史 "config" 数据（explorer-api.ts:2016 区域）。又发现"设置保存后刷新全部变回"根因：`setConfig` 只写 user_option，而前端刷新后从 `user/view/options` 与 `user/index/userInfo` 的 `user.config` 读设置，这两处此前只返回硬编码默认/config_json，未合并 user_option → 保存值刷新即丢；已在这两处合并 `getAllUserOptions(type='')` 覆盖默认（user-api.ts）。编辑器配置同理：前端 `explorer/editor/setConfig`（单 key/value）此前后端无路由、被通配路由吞掉静默失败，且 `/view/options` 的 `editorConfig` 返回空对象 → 编辑器设置同样刷新丢失；已补后端路由存 `user_option(type='editor')`，并在 `/view/options` 返回 `editorConfig` 时合并该类型（explorer-api.ts /editor/setConfig）。头像走 users.avatar 直接读写，无此问题。
